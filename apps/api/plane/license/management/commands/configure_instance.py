        ...
        if not InstanceConfiguration.objects.filter(key__in=keys).exists():
            for key in keys:
                ...
                if key == "IS_OIDC_ENABLED":
                    OIDC_DISCOVERY_URL, OIDC_CLIENT_ID, OIDC_CLIENT_SECRET = get_configuration_value(
                        [
                            {
                    ...
                    if bool(OIDC_DISCOVERY_URL) and bool(OIDC_CLIENT_ID) and bool(OIDC_CLIENT_SECRET):
                        value = "1"
                    else:
                        value = "0"
                    InstanceConfiguration.objects.create(
